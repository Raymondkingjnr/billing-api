...
export async function getCheckoutUrl(userId, planId) {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${process.env.CLIENT_URL}?subscribed=true`,
...
export async function handleWebhook(rawBody, signature) {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      if (!ref) {
        console.warn("[Webhook] Missing client_reference_id");
        break;
      }
      ...
      try {
        ...
        console.error(
          "[Webhook] checkout.session.completed error:",
          err.message,
        );
        throw err;
      }
    ...
    case "invoice.paid": {
      ...
      if (!sub) {
        console.warn(
          "[Webhook] invoice.paid: subscription not in DB yet, checkout.session.completed will handle it",
        );
        break;
      }
    ...
    case "invoice.payment_failed": {
      ...
      console.warn(
        "[Webhook] Payment failed for subscription:",
        inv.subscription,
      );
      break;
    }
